...
import org.devio.hi.ui.item.DataItem
 ...
 */
class LgAdapter(context: Context) : Adapter<ViewHolder>() {
    private val recyclerViewRef: WeakReference<RecyclerView>? = null
    private var mContext: Context = context
    private var mInflater = LayoutInflater.from(context)
    private var dataSets = java.util.ArrayList<DataItem<*, out ViewHolder>>()
    private var typeArrays = SparseArray<DataItem<*, out ViewHolder>>()
     ...
     * 往现有集合的尾部逐年items集合
     */
    fun addItems(items: List<DataItem<*, out ViewHolder>>, notify: Boolean) {
